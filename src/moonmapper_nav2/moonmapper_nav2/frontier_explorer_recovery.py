"""Recovery maneuvers and coverage completion helpers for frontier_explorer."""
...
def coverage_stable(
    ...
    """Return (is_stable, updated_stable_since)."""
    if last_known < 0:
        return False, 0.0
...
class RecoveryRunner:
    """Low-speed cmd_vel recovery when Nav2 is idle (no active goal)."""
    ...
    def tick(self, pose_map: Optional[Tuple[float, float, float]]) -> bool:
        """Returns True when recovery phase finished."""
        if not self._active:
            return True
        now = time.monotonic()
