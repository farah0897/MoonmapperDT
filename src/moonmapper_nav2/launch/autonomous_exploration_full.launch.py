...
#     use_sim_time:=true world_preset:=earth_explore start_sim:=true start_slam:=true \
#     start_nav2:=true start_explorer:=true navigation_stack_delay:=12.0 \
#     explorer_extra_delay_sec:=5.0 delete_rtabmap_db:=false \
#     start_rtabmap_viz:=false start_rviz:=false minimal_nav2_debug:=false \
#     force_set_pose_after_spawn:=false
#
"""Top-level autonomous exploration: sim → RTAB-Map → map relay → Nav2 → frontier V1."""
...
import _nav2_rtabmap_common as _nav2_common  # noqa: E402
...
def _build(context, *args, **kwargs):
    ...
    if params_cli and os.path.isfile(os.path.expanduser(params_cli)):
        ...
        sim_yaml = os.path.join(nav2_share, "config", "nav2_params_rtabmap_sim.yaml")
        real_yaml = os.path.join(nav2_share, "config", "nav2_params_rtabmap_real.yaml")
        params_path = sim_yaml if os.path.isfile(sim_yaml) else real_yaml
    ...
    actions.append(
        LogInfo(
            msg=(
                ...
                "sim included at top-level (not inside OpaqueFunction — required for launch arg scope), "
                f"RTAB-Map @{slam_start_after_launch}s after launch (if enabled), "
                f"Nav2 @{nav_delay}s, explorer @{nav_delay + ex_extra}s "
                "| earth→earth_arena.sdf, earth_explore→earth_arena_explore.sdf"
            )
        )
    )
    actions.append(
...
def generate_launch_description() -> LaunchDescription:
    ...
    if not os.path.isfile(default_params):
        default_params = os.path.join(nav2_share, "config", "nav2_params_rtabmap_real.yaml")
                ...
                actions=[
                    LogInfo(
                        msg=(
                            ...
                            "sim_rover_clean.launch.py not installed — start_sim has no effect"
                        )
                    )
                ],
            )
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                ...
                description=(
                    "moon | earth | earth_explore — forwarded to moonmapper_bringup (→ gazebo_rover). "
                    "earth→earth_arena.sdf; earth_explore→earth_arena_explore.sdf."
                ),
            ),
            DeclareLaunchArgument(
                ...
                default_value="false",
                description=(
                    "Force set robot pose after spawn. Default false for stable autonomous exploration V1."
                ),
            ),
            DeclareLaunchArgument(
                "print_pose_after_spawn",
                default_value="false",
                description="After spawn: print gazebo pose topic once (passed to gazebo_rover). Default false.",
            ),
            DeclareLaunchArgument(
                "sim_stabilization_before_slam_sec",
                default_value="6.0",
                description=(
                    "When start_sim and start_slam are true: seconds after launch to wait "
                    "(sim starts immediately) before starting RTAB-Map."
                ),
            ),
            ...
            DeclareLaunchArgument(
                ...
                description=(
                    "Initial spin publishes here; use /cmd_vel_raw when safety_obstacle bridges "
                    "cmd_vel_raw->/cmd_vel (Nav2 publishes to cmd_vel_raw)."
                ),
            ),
            DeclareLaunchArgument(
                "start_rtabmap_viz",
                default_value="false",
                description="When true, starts RTAB-Map RViz (independent from start_rviz Nav2 RViz).",
            ),
            DeclareLaunchArgument(
                "minimal_nav2_debug",
                default_value="false",
                description="When true, enables Nav2 rviz in nav2_rtabmap_navigation.",
            ),
            *sim_include_actions,
            GroupAction(
                actions=[
