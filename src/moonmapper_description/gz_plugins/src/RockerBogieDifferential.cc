/*
  ...
  Gazebo Sim 8 — valgfri walking-beam + diff-stav for MoonMapper.

  Viktig: Denne koblingen erstatter URDFs lukkede løkke. Den skal IKKE maskere
  feil URDF (kollisjon, feil masse, for stiv hjulkontakt).

  coupling_mode (SDF): off | weak | full
    off  — ingen JointForceCmd (ren passiv URDF/fysikk). Diagnosemodus: URDF/SDF
           modellerer ikke rocker-bogie-differensialen som lukket mekanikk; begge
           rockere kan drive mot samme leddgrense. Ikke normal «kjørbar» modus.
    weak — lav PD på (qL+s*qR) og på diff; anbefalt standard i Gazebo.
    full — sterkere PD (tunable via kp, kd, …); kan oppleves stivt på små rovere.

  debug + print_interval: throttlet gzmsg med leddtilstand og moment (kun når debug=true).
*/
