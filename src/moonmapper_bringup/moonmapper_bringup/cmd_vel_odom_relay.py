...
ROS-standard: ``/cmd_vel.linear.x > 0`` = fremover i ``base_footprint`` +X,
``angular.z > 0`` = mot venstre (positiv yaw-rate).
...
class CmdVelOdomRelay(Node):
    ...
    def __init__(self) -> None:
        ...
        # Skalering inn mot diff_drive (bruk ±1.0). Standard +1/+1: ingen «sign-hack»;
        # ved feil retning fiks URDF hjulakser / L-R lister i diff_drive_controller.yaml.
        self.declare_parameter("cmd_linear_x_sign", 1.0)
        self.declare_parameter("cmd_angular_z_sign", 1.0)
        # Deprecated: True = identisk med cmd_linear_x_sign=-1 og cmd_angular_z_sign=-1 (ignorerer sign-parametre).
