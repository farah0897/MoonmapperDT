"""Nav2 bringup + depth→scan + safety: cmd_vel til /cmd_vel_raw, safety → /cmd_vel.

Ikke kjør samtidig med simple_goal_follower / coverage / waypoint (samme topics).
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    return LaunchDescription(
        [
            LogInfo(
                msg=(
                    "[moonmapper_nav2] Baseline v2: default kart er moonmapper_test_map_padded "
                    "(2 m kant). Overstyr med map_file:=... Ved planner_server-krasj (f.eks. exit -11): "
                    "Ctrl+C og restart hele denne launch."
                )
            ),
            ...
            DeclareLaunchArgument(
                ...
                description="Full sti til kart-.yaml (PGM + yaml).",
            ),
