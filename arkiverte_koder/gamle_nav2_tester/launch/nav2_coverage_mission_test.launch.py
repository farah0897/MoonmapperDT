"""Nav2 baseline + NavigateThroughPoses mission (ingen simple_goal_follower / goal_obstacle).

Kjede: Nav2 → /cmd_vel_raw → safety_obstacle_node → /cmd_vel
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    default_params = os.path.join(nav2_pkg, "config", "nav2_params_moonmapper.yaml")
    default_wp = os.path.join(nav2_pkg, "config", "nav2_coverage_waypoints.yaml")
    ...
    def impl(context):
        ...
        safety = Node(
            ...
            name="safety_obstacle_nav2_mission",
            output="screen",
            parameters=[
                {"use_sim_time": ParameterValue(use_sim_time, value_type=bool)},
                {
        ...
        return [
            LogInfo(
                msg=(
                    "[moonmapper_nav2] Nav2 coverage mission test: NavigateThroughPoses + full Nav2. "
    ...
    return LaunchDescription(
        [
            LogInfo(
                msg=(
                    "[moonmapper_nav2] Baseline: padded kart + static global_costmap (rolling_window=false). "
                    "Ikke kjør med simple_goal_follower / goal_obstacle_avoidance (samme cmd_vel/scan)."
                )
            ),
            ...
            DeclareLaunchArgument(
                ...
                description="Full path to map .yaml (padded default).",
            ),
            DeclareLaunchArgument(
                "params_file",
                default_value=default_params,
                description="Nav2 params yaml for bringup_launch.",
            ),
            DeclareLaunchArgument(
                "waypoints_file",
                default_value=default_wp,
                description="YAML with nav2_mission_client_node ros__parameters.",
            ),
