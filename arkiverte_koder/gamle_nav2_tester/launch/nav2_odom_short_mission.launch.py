"""Nav2 kort misjon i odom: ingen map_server/AMCL, ingen 2D Pose Estimate.

Krever Gazebo/sim allerede kjørende (f.eks. sim_rover_clean). Nav2 bruker rolling costmaps i odom.
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    default_params = os.path.join(nav2_pkg, "config", "nav2_params_odom_test.yaml")
    default_wp = os.path.join(nav2_pkg, "config", "nav2_odom_short_waypoints.yaml")
    odom_rviz = os.path.join(nav2_pkg, "rviz", "moonmapper_nav2_odom_mission.rviz")
    ...
    def impl(context):
        ...
        safety = Node(
            ...
            name="safety_obstacle_nav2_odom_mission",
            output="screen",
            parameters=[
                {"use_sim_time": ParameterValue(use_sim_time, value_type=bool)},
                {
        ...
        mission = Node(
            ...
            name="nav2_odom_mission_client",
            output="screen",
            parameters=[ParameterFile(wp_resolved, allow_substs=False)],
        )

        rviz_node = Node(
            package="rviz2",
            executable="rviz2",
            name="rviz2_nav2_odom_mission",
            output="screen",
        ...
        return [
            LogInfo(
                msg=(
                    "[moonmapper_nav2] ODOM short mission (no AMCL/map_server). "
                    f"params_file={params_resolved}  waypoints_file={wp_resolved}"
                )
            ),
            depth_to_scan,
            bringup,
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                ...
                description="Nav2 params (default: nav2_params_odom_test.yaml).",
            ),
            DeclareLaunchArgument(
                "waypoints_file",
                default_value=default_wp,
                description="Odom-frame waypoint YAML.",
            ),
