...
//Den gjør mange målinger (5 runder × 2 sensorer = 10 bidrag per kanal), legger sammen alle i sum[], og deler på 10. får ett gjennomsnittlig spektrum men begge fysiske sensorene er blandet inn i samme 18-kurven.
...
void AS7265x_Test(float Test[], float D_bakgrunn[], char flagg[], int Tflagg) {
  int rep = 5;
  Tflagg = 1;
...
void loop() {
  ...
  AS7265x_Test(Test, bakgrunn, flagg, Tflagg);
  Serial.println(Tflagg);
  if (Tflagg == 1) {
    for (int i = 0; i < NUM_METALS; i++) {
      met[i] = 4;
    }
