...
This script is intended to work with the processed feature table produced by:
  ml/training/extract_triad_features.py

Input:
- A processed CSV (default: ml/datasets/processed/triad_features.csv)
- A label column (default: label_object)

Behavior:
- Drops non-feature columns automatically (sample_id, file paths, notes, etc.)
- Uses LabelEncoder for string labels and stores it alongside the model
- Train/test split with stratify when possible
- Prints accuracy, classification_report, and confusion_matrix

Outputs:
- ml/models/random_forest_triad.joblib
- ml/models/label_encoder_triad.joblib

Dependencies are NOT installed automatically. If pandas/sklearn/joblib are missing,
the script prints a clear message and exits cleanly.
"""
...
def _select_feature_columns(df, *, label_column: str) -> list[str]:
    """
    Select numeric feature columns from the processed Triad feature CSV.
    Requires pandas.
    """
...
def train_and_evaluate(df, *, label_column: str, random_state: int = 42):
    """
    Train and evaluate a RandomForestClassifier on a dataframe.
...
def main() -> int:
    ...
    try:
        ...
        return 2
    try:
        model, encoder, _, _, _, _ = train_and_evaluate(df, label_column=args.label_column)
    except Exception as exc:  # noqa: BLE001
