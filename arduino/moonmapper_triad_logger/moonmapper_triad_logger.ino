...
static const uint8_t SENSOR0_PORT = 0; // mux-port 1 for sensor 0
static const uint8_t SENSOR1_PORT = 1; // mux-port 2 for sensor 1

static const uint8_t NUM_CHANNELS = 18; // 18 channels per sensor
static const uint8_t NUM_SENSORS = 2; // 2 sensors
static const uint8_t BURST_SAMPLES = 20; // 20 burst samples
static const uint8_t TRIAD_DEVICES[3] = {AS72651_NIR, AS72652_VISIBLE, AS72653_UV};

// et 18 kanall wavelength array fra triaden
...
// start mux-port og initier sensor
bool initSensorOnPort(uint8_t port) {
  ...
  triad.setGain(AS7265X_GAIN_64X); // gain for å øke triad sensivitet
  triad.setIntegrationCycles(18);  // eksponeringstiden per cycle: 2.8ms * cycles. 18 cycles = 50ms.
  triad.disableIndicator();        // slår av status-LED 
  triad.disableBulb(AS72651_NIR); // slår av NIR LED
  triad.disableBulb(AS72652_VISIBLE); // slår av VISIBLE LED
  triad.disableBulb(AS72653_UV); // slår av UV LED
  return true;
}

// leser LED_ON - LED_OFF i diff[18], og trekker backgroundDiff[sensorIdx] hvis bakgrunn er klar.
...
void printCsvRow(const String &sampleId, uint8_t burstIndex, unsigned long ts, const float s0[NUM_CHANNELS], const float s1[NUM_CHANNELS]) {
  ...
  // skriver ut rå korrigerte verdier for S0 og S1
  for (uint8_t i = 0; i < NUM_CHANNELS; i++) {
    Serial.print(",");
    Serial.print(s0[i], 6);
  }
...
// skriver ut en måling per sensor
